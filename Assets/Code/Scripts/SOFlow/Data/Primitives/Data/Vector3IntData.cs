...
namespace SOFlow.Data.Primitives
{
    ...
    public class Vector3IntData : PrimitiveData
    {
        ...
        {
            if(value.Value is Vector3Int)
                Value = (Vector3Int)value.Value;
            else if(value.Value is Vector3IntData)
                Value = ((Vector3IntData)value.Value).Value;
            else
                Debug.LogWarning($"[Vector3IntData] Supplied value is not a supported data type.\n{name}");
        }
