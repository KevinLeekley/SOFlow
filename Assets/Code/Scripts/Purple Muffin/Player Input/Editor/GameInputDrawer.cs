...
namespace PurpleMuffin.PlayerInput
{
    ...
    public class GameInputDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            DrawKeyList(position, keys, PMEditorSettings.TertiaryLayerColour);
        ...
        private void DrawKeyList(Rect position, SerializedProperty property, Color layerColour)
        {
            ...
            GUI.backgroundColor = PMEditorSettings.AcceptContextColour;

            if(GUI.Button(position, property.isExpanded ? "↑" : "↓")) property.isExpanded = !property.isExpanded;

            GUI.backgroundColor = originalColour;

            Vector2 textSize = PMStyles.BoldCenterLabel.CalcSize(new GUIContent(property.displayName));

            position.width =  textSize.x;
            position.x     += originalRect.width / 2f - textSize.x / 2f;

            GUI.Label(position, property.displayName, PMStyles.BoldCenterLabel);

            textSize = PMStyles.WordWrappedMiniLabel.CalcSize(new GUIContent($"Size: {keyCount}"));

            position.width = textSize.x;
            position.x     = originalRect.width;

            GUI.Label(position, $"Size: {keyCount}", PMStyles.WordWrappedMiniLabel);
            ...
            if(property.isExpanded)
                for(int i = 0; i < keyCount; i++)
                {
                    ...
                    textSize = PMStyles.Label.CalcSize(new GUIContent("Key"));

                    position.width = textSize.x;

                    GUI.Label(position, "Key", PMStyles.Label);
                    ...
                    textSize = PMStyles.Label.CalcSize(new GUIContent("Input Type"));

                    position.x     = originalRect.width / 2f + indentSpace;
                    position.width = textSize.x;

                    GUI.Label(position, "Input Type", PMStyles.Label);
                    ...
                    GUI.backgroundColor = PMEditorSettings.DeclineContextColour;
