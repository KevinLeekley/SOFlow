...
namespace PurpleMuffin.Internal.SceneManagement
{
    public class SceneManagerWindow : EditorWindow
    {
        ...
        [MenuItem("PurpleMuffin/Scene Manager")]
        public static void ShowWindow()
        {
            GetWindow<SceneManagerWindow>("PM-Scene Manager");
        }

        private void OnGUI()
        {
            PMEditorUtilities.DrawHorizontalColourLayer(PMEditorSettings.PrimaryLayerColour, DrawHeaderPanel);
            ...
            if(_selectedSceneSet > 0)
            {
                ...
                PMEditorUtilities.DrawHorizontalColourLayer(PMEditorSettings.PrimaryLayerColour, DrawFooterPanel);
            }
        }
        ...
        private void DrawHeaderPanel()
        {
            ...
            if(PMEditorUtilities.DrawColourButton("Refresh", Color.cyan))
            {
                _sceneSets.Clear();
                _scenes.Clear();
        ...
        {
            if(PMEditorUtilities.DrawColourButton("Load Scene Set", PMEditorSettings.AcceptContextColour))
            {
            ...
            if(PMEditorUtilities.DrawColourButton("Unload Scene Set", PMEditorSettings.DeclineContextColour))
        ...
        private void DrawSceneEntry(SceneField scene)
        {
            ...
            if(PMEditorUtilities.DrawColourButton("Load", PMEditorSettings.AcceptContextColour))
                EditorSceneManager.OpenScene(AssetDatabase.GetAssetPath(scene.SceneAsset), OpenSceneMode.Additive);

            if(PMEditorUtilities.DrawColourButton("Replace", Color.yellow))
                EditorSceneManager.OpenScene(AssetDatabase.GetAssetPath(scene.SceneAsset), OpenSceneMode.Single);

            if(PMEditorUtilities.DrawColourButton("Unload", PMEditorSettings.DeclineContextColour))
                EditorSceneManager
